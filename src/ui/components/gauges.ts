...
function pcScale(pc: number, tone: Tone): SVGSVGElement {
  ...
  const svg = s('svg', { class: `pcscale tone-${tone}`, viewBox: '0 0 160 34', role: 'img', 'aria-label': `Political capital ${Math.round(pc)} of 100. Resignation at 0.` });
  svg.append(
    ...
    s('text', { class: 'pc-text', x: 0, y: 30.5 }, 'resigns'),
    s('text', { class: 'pc-text', x: 160, y: 30.5, 'text-anchor': 'end' }, '100'),
  );
  return svg;
}
