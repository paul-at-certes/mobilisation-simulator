...
export const ACTION_COPY: Record<ActionId, ActionCopy> = {
  ...
  introduce_bill: {
    ...
    html: `Starts the legislative clock: ${s('bill_months_emergency')} months under emergency procedure or ${s('bill_months_normal')} under the normal timetable. The Bill's clauses set the age band, whether women are included (${s('women_included_support_pct')} of the public say yes), the medical standard and the exemptions regime. <strong>The age band decides more than the size of the pool:</strong> the people you conscript are the people who object, and support runs from 27% among 18–24s to 63% among the over-65s, so a band that leaves the 18–24s out is a more willing one and fewer of those called refuse to report. It is also the band that takes people out of settled work, which costs capital to legislate and shows up later in the training figures. Nothing else in this section is available until it passes.`,
  },
  amend_bill: {
    group: 'conscription',
    title: 'Amend the Bill',
    html: `Change the age band, the inclusion of women, the medical standard or the exemptions. Costs an action and any political capital the new clause carries — for the age band, the difference between the two, so narrowing it refunds. Anyone already on a course keeps the terms they were called up under.`,
  },
  set_callup: {
    group: 'conscription',
  ...
  raise_spending: {
    ...
    html: `Doubles the Treasury allowance the mobilisation may run up before it starts charging political capital (currently ${s('cost_pc_allowance_per_month')} for every month of the deadline; each whole allowance thereafter costs ${s('cost_pc_penalty_per_step')} a month).`,
  },
  blame_predecessors: {
    group: 'political',
